...
class Resource
{
	...
	public function __construct(Request $request, string $path, string $description, array $parameters = [], string $type = Request::METHOD_GET)
	{
		$this->path = $path;
		$this->type = $type;
	...
	public function getResource(): array
	{
		...
		return [
			$this->path => ['type' => $this->type, 'description' => $this->description, 'parameters' => $parameters,
	...
	private function getParameterForEndpointExample(array $parameter): string
	{
		...
		$param .= match ($parameter['type']) {'int' => random_int(1, 31),'bool' => (bool) rand(0, 1),

...
class RootTest extends APITests
{
    ...
    public function testOptions(): void
    {
        ...
        $this->withHeaders($this->getHeader())->options($this->domain . '/allows')->assertHeader(
            'Allow',
            implode(', ', $methods)
        );
    }
    ...
    {
        return [[null], [Token::GUEST_ROLE], [Token::ADMIN_ROLE], [Token::RECRUITER_ROLE], [Token::TECHNICIAN_ROLE]];
    }
    ...
    private function getAuthorization(?string $role): array
    {
        ...
        $bearerToken = match ($role) {
            ...
            Token::TECHNICIAN_ROLE => $bearerToken . 'test_technician_token',
            default => '',
        };
    ...
    private function assertPublicResources(AssertableJson $resources): AssertableJson
    {
        return $resources->has(
            ...
            fn(AssertableJson $token) => $token->hasAll($this->resourceIndexes)->where(
                ...
                self::DESCRIPTION_INDEX,
                __(
                    self::TOKEN_TRANSLATIONS . 'request'
                )
            )->has(self::PARAMETER_INDEX, 2)->has(
                self::PARAMETER_INDEX . '.0',
                fn(AssertableJson $parameter) => $parameter->hasAll($this->parameterIndexes)->where(
                    self::NAME_INDEX,
                    'username'
                )->where(
                    self::TYPE_INDEX,
                ...
                self::ENDPOINT_INDEX,
                $this->domain . '/tokens?username=username&password=password'
            )
        )->has(
            'accounts',
            fn(AssertableJson $account) => $account->hasAll($this->resourceIndexes)->where(
                self::TYPE_INDEX,
                Request::METHOD_POST
            )->where(
                self::DESCRIPTION_INDEX,
                __(
                    self::ACCOUNT_TRANSLATIONS . 'request'
                )
            )->has(self::PARAMETER_INDEX, 4)->has(
                self::PARAMETER_INDEX . '.0',
                fn(
                    AssertableJson $parameter
                ) => $parameter->hasAll($this->parameterIndexes)->where(
                    self::NAME_INDEX,
                    'email'
                )->where(
                    self::TYPE_INDEX,
                ...
                self::ENDPOINT_INDEX,
                $this->domain . '/accounts?email=email&name=name&language=language&linkedin_profile=linkedin_profile'
            )
        );
    }
    ...
    private function assertTokenedResources(AssertableJson $resources): AssertableJson
    {
        return $resources->has(
            ...
            fn(AssertableJson $token) => $token->hasAll($this->resourceIndexes)->where(
                ...
                self::DESCRIPTION_INDEX,
                __(
                    self::TOKEN_TRANSLATIONS . 'refresh'
                )
            )->where(
                self::ENDPOINT_INDEX,
                $this->domain . '/tokens'
